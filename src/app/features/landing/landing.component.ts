...
@Component({
  ...
  styles: [`
    ...
    @keyframes glowAmber {
      0%, 100% { box-shadow: 0 0 24px rgba(196,108,42,.45), 0 0 60px rgba(196,108,42,.15); }
      50%       { box-shadow: 0 0 40px rgba(196,108,42,.65), 0 0 90px rgba(196,108,42,.25); }
    }
    ...
    @keyframes mistDrift {
      ...
      15%  { opacity: .6; }
      85%  { opacity: .4; }
      100% { transform: translateX(5%) scaleX(1.04); opacity: 0; }
    }

    /* ─── Amber text ────────────────────────────────────────────── */
    .amber-text {
      background: linear-gradient(90deg,
        #8b4c18 0%, #c4742a 20%, #e8953f 42%, #f2b05e 55%, #e8953f 70%, #c4742a 85%, #8b4c18 100%);
    ...
    .btn-cta:hover {
      ...
      box-shadow: 0 14px 42px rgba(196,108,42,.35), 0 0 60px rgba(58,116,72,.25);
    }
    ...
    .feat-card:hover {
      ...
      border-color: rgba(196,108,42,.35);
    ...
    .stat-card:hover {
      ...
      border-color: rgba(92,158,112,.3);
    ...
    .premium-wrapper {
      position: relative; border-radius: 1rem; padding: 1px; overflow: hidden;
    }
    .premium-spin {
      position: absolute; inset: -60%;
      background: conic-gradient(
        from 0deg, transparent 0deg,
        rgba(196,108,42,.0) 60deg, rgba(196,108,42,.8) 120deg,
        rgba(232,149,63,.9) 180deg, rgba(196,108,42,.8) 240deg,
        rgba(196,108,42,.0) 300deg, transparent 360deg
      );
    ...
    .premium-inner {
      ...
      background: linear-gradient(145deg, #0e2010 0%, #0a1a0c 50%, #081208 100%);
    }

    /* ─── Step number ───────────────────────────────────────────── */
    .step-num {
      background: linear-gradient(135deg, #2a5c35, #3a7448);
      box-shadow: 0 8px 28px rgba(42,92,53,.5);
    }
    ...
    .ripple-ring {
      position: absolute;
      border-radius: 50%;
      border: 1.5px solid rgba(92,158,112,.5);
      animation: ripple 4s ease-out infinite;
    }
    .ripple-ring-2 {
      position: absolute;
      border-radius: 50%;
      border: 1px solid rgba(196,108,42,.3);
      animation: ripple2 5s ease-out infinite;
    }
  ...
  template: `
    ...
    <!-- ════════════════════════════════════════════════════════════
         HERO — ambiance aube sur la rivière
    ═════════════════════════════════════════════════════════════ -->
      ...
      <div class="relative max-w-6xl mx-auto px-5 flex flex-col lg:flex-row items-center gap-12 lg:gap-8"
        ...
        <div class="flex-1 max-w-xl lg:max-w-none lg:flex-[1.1]">
          ...
          <div class="a-fadeUp inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full glass text-xs font-semibold mb-8"
               style="color:rgba(196,108,42,1);">
            <span class="w-1.5 h-1.5 rounded-full animate-pulse" style="background:#c4742a;"></span>
            Gratuit · 50 captures offertes
          </div>
          ...
          <p class="a-fadeUp-2 leading-relaxed max-w-md mb-10"
            ...
            Enregistre chaque prise, découvre 200&nbsp;+ espèces, suis ta progression et partage tes sorties avec d'autres passionnés.
          </p>
    ...
    <section style="background:linear-gradient(180deg,#0e1e10 0%,#0c1a0d 100%);" class="py-24">
      <div class="max-w-5xl mx-auto px-5">
        <div class="text-center mb-16">
          <p class="text-xs font-bold uppercase tracking-[4px] mb-5" style="color:#5c9e70;">Tarifs</p>
          <h2 class="text-4xl md:text-5xl font-black tracking-tight" style="color:#f0ebe2;">
            Commence <span style="color:#5c9e70;">gratuitement.</span>
          </h2>
          <p class="mt-4" style="color:rgba(240,235,226,.35);">Pas de carte bancaire. Pas d'engagement.</p>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-3xl mx-auto">
          ...
          <div class="glass rounded-2xl p-8">
            <p class="text-xs font-bold uppercase tracking-widest mb-4" style="color:rgba(240,235,226,.32);">Freemium</p>
            <div class="flex items-end gap-2 mb-1">
              <span class="text-6xl font-black tracking-tighter" style="color:#f0ebe2;">0€</span>
            </div>
            <p class="text-sm mb-8" style="color:rgba(240,235,226,.26);">Pour toujours</p>
            <ul class="space-y-4 mb-8">
              @for (item of freeFeatures; track item) {
                <li class="flex items-center gap-3 text-sm" style="color:rgba(240,235,226,.52);">
                  <span class="w-5 h-5 rounded-full flex items-center justify-center text-xs font-black flex-shrink-0"
                        style="background:rgba(42,92,53,.35);color:#5c9e70;">✓</span>
                  {{ item }}
                </li>
              }
            </ul>
            <a routerLink="/register"
               class="glass block w-full text-center py-4 text-sm font-semibold rounded-xl transition-all"
               style="color:rgba(240,235,226,.7);"
               onmouseenter="this.style.color='rgba(240,235,226,.95)'"
               onmouseleave="this.style.color='rgba(240,235,226,.7)'">
              Créer un compte
            </a>
          </div>

          <!-- Premium (bordure ambrée tournante) -->
          <div class="premium-wrapper">
            <div class="premium-spin"></div>
            <div class="premium-inner p-8">
              <div class="flex items-center justify-between mb-4">
                <p class="text-xs font-bold uppercase tracking-widest amber-text">Premium</p>
                <span class="text-xs font-bold px-3 py-1 rounded-full"
                      style="background:rgba(196,108,42,.12);color:#e8953f;border:1px solid rgba(196,108,42,.3);">Populaire</span>
              </div>
              <div class="flex items-end gap-2 mb-1">
                <span class="text-6xl font-black tracking-tighter" style="color:#f0ebe2;">3€</span>
                <span class="text-sm mb-2.5" style="color:rgba(240,235,226,.32);">/mois</span>
              </div>
              <p class="text-sm mb-8" style="color:rgba(240,235,226,.26);">Annulable à tout moment</p>
              <ul class="space-y-4 mb-8">
                @for (item of premiumFeatures; track item) {
                  <li class="flex items-center gap-3 text-sm" style="color:rgba(240,235,226,.62);">
                    <span class="w-5 h-5 rounded-full flex items-center justify-center text-xs font-black flex-shrink-0 amber-text"
                          style="background:rgba(196,108,42,.15);">✓</span>
                    {{ item }}
                  </li>
                }
              </ul>
              <a routerLink="/register" class="btn-cta block w-full text-center py-4 text-sm font-bold text-white rounded-xl">
                Essayer Premium →
              </a>
            </div>
          </div>
    ...
    <section class="relative overflow-hidden py-36" style="background:#0a1509;">
      ...
      <div class="relative max-w-2xl mx-auto px-5 text-center">
        ...
        <p class="text-lg mb-10" style="color:rgba(240,235,226,.36);">
          Rejoins FishDex. Commence à construire ton journal de pêche dès aujourd'hui.
        </p>
...
export class LandingComponent {
  ...
  // Ronds d'eau déterministes (pas de Math.random)
  ripples = Array.from({ length: 8 }, (_, i) => {
    ...
    return {
      id:  i,
      cls: isAmber ? 'ripple-ring-2' : 'ripple-ring',
  ...
  features = [
    { icon: '📸', title: 'Journal photo',  desc: 'Capture le moment : photo, poids, taille et GPS en quelques secondes.',                                    bg: 'rgba(92,158,112,.1)',  border: 'rgba(92,158,112,.2)'  },
    { icon: '🐟', title: 'Encyclopédie',   desc: '200+ espèces avec habitats, calendriers de saison et conseils de pêche détaillés.',                        bg: 'rgba(56,189,248,.07)', border: 'rgba(56,189,248,.14)' },
    { icon: '🏆', title: 'Badges & défis', desc: 'Débloque des badges au fil de tes prises. Premier brochet, record, naturaliste…',                          bg: 'rgba(196,108,42,.1)',  border: 'rgba(196,108,42,.2)'  },
    { icon: '📊', title: 'Statistiques',   desc: 'Poids total, espèces, records, mois actifs — toute ta progression en un coup d\'œil.',                    bg: 'rgba(167,139,250,.07)',border: 'rgba(167,139,250,.15)' },
    { icon: '👥', title: 'Groupes',        desc: 'Rejoins des clubs, partage tes sorties et consulte le fil de tes amis en temps réel.',                     bg: 'rgba(251,146,60,.07)', border: 'rgba(251,146,60,.14)'  },
    { icon: '📍', title: 'Spots GPS',      desc: 'Enregistre l\'emplacement exact de tes spots et retrouve-les à chaque sortie.',                            bg: 'rgba(248,113,113,.07)',border: 'rgba(248,113,113,.14)' },
  ];
  ...
  freeFeatures    = ['50 captures maximum', 'Encyclopédie complète (200+ espèces)', 'Badges de base', '1 groupe de pêche'];
