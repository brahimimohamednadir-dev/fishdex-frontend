  ...
  template: `
    <div class="max-w-6xl mx-auto px-5 py-8">
      ...
      <div class="flex items-end justify-between mb-8">
        <div>
          <h1 class="text-2xl font-bold text-gray-900 tracking-tight">Mes captures</h1>
          @if (page) {
        ...
        <a routerLink="/captures/new"
           class="flex items-center gap-2 px-4 py-2 bg-gray-900 text-white text-sm font-semibold rounded-lg hover:bg-gray-700 transition-all">
      ...
      @if (loading) {
        ...
        <!-- Empty state -->
        <div class="text-center py-24 border-2 border-dashed border-gray-200 rounded-2xl">
          <span class="text-5xl opacity-30">🎣</span>
          <h2 class="mt-4 text-lg font-semibold text-gray-900">Aucune capture pour l'instant</h2>
          <p class="mt-1 text-sm text-gray-500">Enregistre ta première prise et commence ton journal.</p>
          <a routerLink="/captures/new"
             class="inline-flex items-center gap-2 mt-6 px-5 py-2.5 bg-gray-900 text-white text-sm font-semibold rounded-lg hover:bg-gray-700 transition-all">
            Ajouter une capture
          </a>
        </div>
      } @else {
        ...
        @if ((page?.totalPages ?? 0) > 1) {
          <div class="flex justify-center items-center gap-2 mt-10">
            <button (click)="loadPage(currentPage - 1)" [disabled]="currentPage === 0"
                    class="px-4 py-2 text-sm font-medium text-gray-600 bg-white border border-gray-200 rounded-lg hover:bg-gray-50 disabled:opacity-30 disabled:cursor-not-allowed transition-all">
              ← Précédent
            </button>
