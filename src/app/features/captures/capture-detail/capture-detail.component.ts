      ...
      @else if (capture) {
        <div class="mt-6 space-y-4">
          ...
          <div class="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm">
            <p class="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-3">Photo</p>
            <app-photo-upload
          ...
          <!-- Titre + date -->
          <div class="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm">
            <h1 class="text-2xl font-bold text-gray-900 tracking-tight">{{ capture.speciesName }}</h1>
            ...
            <div class="grid grid-cols-2 gap-3 mt-4">
              <div class="bg-gray-50 rounded-xl p-4">
                <p class="text-xs font-semibold text-gray-400 uppercase tracking-wide">Poids</p>
                <p class="text-2xl font-bold text-gray-900 mt-1">
                  {{ capture.weight }}<span class="text-sm font-medium text-gray-500 ml-1">kg</span>
                </p>
              </div>
              <div class="bg-gray-50 rounded-xl p-4">
                <p class="text-xs font-semibold text-gray-400 uppercase tracking-wide">Taille</p>
                <p class="text-2xl font-bold text-gray-900 mt-1">
                  {{ capture.length }}<span class="text-sm font-medium text-gray-500 ml-1">cm</span>
                </p>
              </div>
            </div>
          </div>
          ...
          @if (capture.note || capture.latitude || capture.species) {
            <div class="bg-white border border-gray-100 rounded-2xl shadow-sm divide-y divide-gray-50">
              @if (capture.note) {
                <div class="p-5">
                  <p class="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-1">Note</p>
                  <p class="text-sm text-gray-700 leading-relaxed">{{ capture.note }}</p>
                </div>
              }
              @if (capture.latitude && capture.longitude) {
                <div class="p-5">
                  <p class="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-1">GPS</p>
              ...
              @if (capture.species) {
                <div class="p-5 flex items-center justify-between">
                  <p class="text-xs font-semibold text-gray-400 uppercase tracking-wide">Espèce</p>
